        ...
        public override void Run()
        {
            ...
            else
            {
                if (Int32.TryParse(VirtualMachine.Stack.Peek().ToString(), out stackValue))
                {
                    ...
                    {
                        throw new SvmRuntimeException("Equint value was not an integer. Instruction given: equint " 
                            + Operands[0].ToString());
                    }
                }
                else
                {
                    throw new SvmRuntimeException("Value at the top of the stack was not an integer. Top stack value: " 
                        + VirtualMachine.Stack.Peek());
                }
            }
        }
    }
